...
(function(){
  ...
  function buildManual(){const lines=cleanLines(listBox.value);if(!lines.length){window.AutoCartUI?.setStatus?.('Enter at least one item.','warn');return}window.AutoCartUI?.render?.(AutoCartCore.buildListPlan(lines,retailer.value))}
  ...
  async function importFile(){
    ...
    const form=new FormData();form.append('file',file,file.name);form.append('retailer',importRetailer.value);
    try{const r=await fetch(api+'/api/import',{method:'POST',body:form});const d=await r.json();if(!r.ok||!d.ok)throw new Error(d.error||'Import failed');window.AutoCartUI?.render?.(AutoCartCore.normalizeImportedPlan(d,importRetailer.value));fileStatus.textContent='Imported '+file.name+'. Review every item before retailer handoff.'}catch(e){fileStatus.textContent='Import failed: '+e.message}
  }
