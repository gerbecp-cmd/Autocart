(function(root,factory){
  ...
  const ADDITIONS={
    ...
    acehardware:{label:'Ace Hardware',aliases:['ace hardware','ace'],base:'https://www.acehardware.com/search?query=',mode:'query'},
  ...
  Object.entries(ADDITIONS).forEach(([id,meta])=>{
    core.RETAILER_META[id]=meta;
    if(!core.RETAILERS.includes(id))core.RETAILERS.push(id);
  });
  ...
  function detect(text){
    ...
    const matches=[];
    Object.entries(core.RETAILER_META).forEach(([id,meta])=>meta.aliases.forEach(a=>{if(t.includes(String(a).toLowerCase()))matches.push({id,len:a.length})}));
    matches.sort((a,b)=>b.len-a.len);
    return matches[0]?.id||null;
  }
  function escapeRegExp(s){return String(s).replace(/[.*+?^${}()|[\]\\]/g,'\\$&')}
  function setRetailerInCommand(text,id){
    if(!core.RETAILER_META[id])return String(text||'');
    let out=String(text||'').trim();
    const aliases=[];
    Object.values(core.RETAILER_META).forEach(m=>m.aliases.forEach(a=>aliases.push(a)));
    aliases.sort((a,b)=>b.length-a.length).forEach(a=>{out=out.replace(new RegExp('\\b(?:at|from|to)?\\s*'+escapeRegExp(a)+'\\b','ig'),' ').replace(/\s+/g,' ').trim()});
    return (out+' at '+core.retailerLabel(id)).trim();
  }

  const oldNormalizeRemote=core.normalizeRemote;
  core.normalizeRemote=function(data,text){
    const plan=oldNormalizeRemote(data,text);const explicit=detect(text);if(plan?.ok&&explicit)plan.retailer=explicit;return plan;
  };
  const oldNormalizeImported=core.normalizeImportedPlan;
  core.normalizeImportedPlan=function(data,retailer){
    const plan=oldNormalizeImported(data,retailer);if(plan?.ok&&core.RETAILER_META[retailer])plan.retailer=retailer;return plan;
  };

  function populateCommandPicker(){
    if(!doc)return;
    const select=doc.getElementById('commandRetailer');
    if(select){
      select.innerHTML='<option value="">Use store named in command (default Walmart)</option>';
      Object.entries(GROUPS).forEach(([name,ids])=>{
        const group=doc.createElement('optgroup');group.label=name;
        ids.filter(id=>core.RETAILER_META[id]).forEach(id=>{const o=doc.createElement('option');o.value=id;o.textContent=core.retailerLabel(id);group.appendChild(o)});
    ...
    if(directory)directory.innerHTML=Object.entries(GROUPS).map(([name,ids])=>`<div class="store-group"><strong>${name}</strong><div>${ids.filter(id=>core.RETAILER_META[id]).map(id=>core.retailerLabel(id)).join(' · ')}</div></div>`).join('');
  }
